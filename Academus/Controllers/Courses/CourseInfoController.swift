...
//        let percent = UILabel().setUpLabel(text: "\(model?.grade?.percent ?? 0.0)", font: UIFont.largeHeader!, fontColor: .navigationsGreen)
//        percent.font = UIFont(name: "AvenirNext-demibold", size: 48)
//        percent.sizeToFit()
//        let percentTitle = UILabel().setUpLabel(text: "Percent", font: UIFont.standard!, fontColor: .tableViewLightGrey)
...
//        percent.anchors(top: courseName.bottomAnchor, topPad: 0, left: gradeLabel.rightAnchor, leftPad: 18)
    ...
    private func graphCell(manager: CourseInfoCellManager, cell: UITableViewCell) -> UITableViewCell {
        ...
        graph.xAxis.labelTextColor = .navigationsGreen
